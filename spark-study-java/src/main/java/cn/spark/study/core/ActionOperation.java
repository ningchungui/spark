...
public class ActionOperation {
    ...
    private static void foreach() {
        SparkConf conf = new SparkConf().setAppName("foreach").setMaster("local");
        JavaSparkContext sc = new JavaSparkContext(conf);

        List<String> datas = Arrays.asList("{'id':1,'name':'xl1','pwd':'xl123','sex':2}",
                "{'id':2,'name':'xl2','pwd':'xl123','sex':1}",
                "{'id':3,'name':'xl3','pwd':'xl123','sex':2}");

        JavaRDD<String> datasRDD = sc.parallelize(datas);
        datasRDD.foreach(new VoidFunction<String>() {
            @Override
    ...
    public static void mapPartitions() {
        SparkConf conf = new SparkConf().setAppName("mapPartitions")
                .setMaster("local");
        ...
        JavaRDD<String> mapPartitionsRDD = namesRDD.mapPartitions(
                new FlatMapFunction<Iterator<String>, Iterator<String>>() {
                    int count = 0;
    ...
    public static void mapPartitionsWithIndex(){
        SparkConf conf = new SparkConf().setAppName("mapPartitionsWithIndex")
                .setMaster("local");
